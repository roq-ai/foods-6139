...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'PUT':
      return updateDeliveryById();
    case 'DELETE':
      return deleteDeliveryById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getDeliveryById() {
    const data = await prisma.delivery.findFirst(convertQueryToPrismaUtil(req.query, 'delivery'));
    return res.status(200).json(data);
  }

  async function updateDeliveryById() {
    await deliveryValidationSchema.validate(req.body);
